      ...
      case MZ_IDC_TOOLBARPRO:
          {
              ...
              if (nIndex == TOOLBARPRO_LEFT_TEXTBUTTON)
              {
                  ...
                  {
                      // 创建Widget对象
                      m_pSmsWidget = GetWidget(L"\\windows\\SmsWidget.dll");
                      int nW = 0;
                      int nH = 0;
                      m_pSmsWidget->OnCalcItemSize(nW, nH);
                      m_pSmsWidget->SetPos(50, 50, DESKTOPITEM_WIDTH * nW, DESKTOPITEM_HEIGHT * nH);

                      // 把Widget添加到窗口中
                      AddUiWin(m_pSmsWidget);
                      // 启动Widget
                      m_pSmsWidget->StartWidget();
                  }
              }
              ...
              else if (nIndex == TOOLBARPRO_RIGHT_TEXTBUTTON)
              {
                  ...
                  {
                      // 无效控件所在区域
                      m_pSmsWidget->Invalidate(NULL);
                      // 从窗口中移除指定的Widget
                      RemoveUiWin(m_pSmsWidget);
                      // 更新控件所在窗口
                      m_pSmsWidget->Update();

                      m_pSmsWidget->EndWidget();
                      delete m_pSmsWidget;
                      m_pSmsWidget = NULL;
                  }
              }
